	...
	if err == OK:
		...
		if book_key=="jpg":
			...
			while not book.is_empty():
				...
				Data.comic_array.append(ImageTexture.create_from_image(book_image))				
				start_num += 1
...
func 开始阅读():
	if start_read:
		if Data.comic_array.size() >= now_page + 5 or end_read:
...
#初始加载
func _ready() -> void:
	now_page = Data.comic.get(comic_name,0)
